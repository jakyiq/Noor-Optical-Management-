...
<div class="rx-slip" style="box-sizing:border-box;width:100%;padding:10px 12px;font-size:.842rem;line-height:1.45">
  ...
  <!-- RX TABLE -->
  <table style="width:100%;margin-bottom:8px;border-collapse:collapse;table-layout:fixed;font-size:.76rem">
    <colgroup>
      <col style="width:16%">
      <col style="width:13%">
      <col style="width:13%">
    ...
    <thead><tr style="background:var(--burgundy);color:white">
      <th style="padding:4px 4px;text-align:left">Eye</th>
      <th style="padding:4px 3px;text-align:center">SPH</th>
      <th style="padding:4px 3px;text-align:center">CYL</th>
      <th style="padding:4px 3px;text-align:center">AXIS</th>
      <th style="padding:4px 3px;text-align:center">ADD</th>
      <th style="padding:4px 3px;text-align:center">VA</th>
      <th style="padding:4px 3px;text-align:center">BCVA</th>
    </tr></thead>
    <tbody>
      <tr style="background:var(--cream-bg)">
        <td style="padding:4px 4px;font-weight:700;font-size:.72rem;border:1px solid var(--cream-border);white-space:nowrap">OD (R)</td>
        ${[v.od_sphere,v.od_cylinder,v.od_axis,v.od_addition,v.od_va,v.od_bcva].map(x=>`<td style="text-align:center;padding:4px 3px;border:1px solid var(--cream-border);word-break:break-word;overflow-wrap:break-word">${esc(x??'—')}</td>`).join('')}
      </tr>
      <tr>
        <td style="padding:4px 4px;font-weight:700;font-size:.72rem;border:1px solid var(--cream-border);white-space:nowrap">OS (L)</td>
        ${[v.os_sphere,v.os_cylinder,v.os_axis,v.os_addition,v.os_va,v.os_bcva].map(x=>`<td style="text-align:center;padding:4px 3px;border:1px solid var(--cream-border);word-break:break-word;overflow-wrap:break-word">${esc(x??'—')}</td>`).join('')}
      </tr>
      <tr style="background:var(--cream-bg)">
        <td style="padding:4px 4px;font-weight:700;font-size:.72rem;border:1px solid var(--cream-border);white-space:nowrap;color:var(--burgundy)">IPD</td>
        <td colspan="6" style="text-align:center;padding:4px 3px;border:1px solid var(--cream-border);font-family:'Figtree','DM Sans',sans-serif;font-weight:600">${(v.ipd != null && v.ipd !== '') ? esc(v.ipd) + ' mm' : '—'}</td>
      </tr>
    </tbody>
  </table>
...
function _buildRxPrintBlob() {
  ...
  const printCSS = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    html{background:white;font-family:'Cairo','Noto Sans Arabic','DM Sans',sans-serif;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;text-rendering:optimizeLegibility;}
    body{background:white;width:130mm;margin:0 auto;overflow:visible;}
    .rx-slip{background:white;border:none;padding:0;width:100%;font-size:9pt;line-height:1.5;}
    .rx-slip-label{font-size:6pt;text-transform:uppercase;letter-spacing:.08em;color:${sc.text}88;font-weight:600}
    ...
    th,td{border:1px solid ${sc.border};padding:4px 4px;word-break:break-word;overflow-wrap:break-word;white-space:normal}
